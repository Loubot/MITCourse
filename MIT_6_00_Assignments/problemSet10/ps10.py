...
def graphRemovedErr(points, kvals = [25, 50, 75, 100, 125, 150], cutoff = 0.1):
    ...
    """
    trainingSet , holdOutSet = randomPartition(points, .8)
    errorSet = []

    holdOutErrorSet = []
    for val in kvals:

        clusterSet , maxDist = kmeans(trainingSet,val, cutoff, County)
        ...
        for cluster in clusterSet:
            ...
            for point in points:
                errorSummation += (point.distance(centroid))**2
##        print 'error',errorSummation
        errorSet.append(errorSummation)
        ...
        for point in holdOutSet:
            ...
            for cluster in clusterSet:
                centroid = cluster.computeCentroid()
                if point.distance(centroid)<minDist:
                    minDist = point.distance(centroid)

            holdOutError+= minDist **2
        holdOutErrorSet.append(holdOutError)
    ratioSet = []
    for i in range(0, len(holdOutErrorSet)):
        ratioSet.append(holdOutErrorSet[i]/ errorSet[i])

    pylab.figure()
    pylab.plot(kvals,errorSet)
    pylab.plot(kvals, holdOutErrorSet)
    pylab.figure()
    pylab.plot(ratioSet)
    pylab.show()
    ...
    for i in range(0,3):
        ...
        for cluster in clusterSet:
            if cluster.contains(county):
                print 'Iteration: '+ str(i +1) + ' = '  + str(cluster)
